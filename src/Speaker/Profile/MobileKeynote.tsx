...
import Speaker1 from "../../Assets/Speakers/PHILRICE_SPEAKER_MARCOS.png";
    ...
    <div className="flex flex-col items-center justify-start mt-10 text-sm md:text-2xl">
      ...
      {/* <div className="flex flex-wrap justify-center gap-[200px] md:gap-[100px] lg:gap-[80px]">
        {speakersData.map((speaker, index) => (
          <div
            key={index}
            className="w-[299px] md:w-[449px] h-[370px] md:h-[518px] bg-custom-gradient rounded-tr-[100px] relative -mt-40 md:mt-2 lg:mt-0 cursor-pointer transform transition-transform duration-300 hover:scale-105 flex flex-row"
            onClick={() => openModal(speaker)}>
            <img
              src={speaker.image}
              alt={speaker.name}
            ...
            />
            <div className="w-40 h-2 bg-[#EFB71E]" />
            <div className="text-[15px] md:text-[22px] bottom-0 flex flex-col justify-center items-center text-center text-white bg-[#0E9046] w-full h-[115.64px] md:h-[147px] rounded-bl-[80px] px-2">
              His Excellency
